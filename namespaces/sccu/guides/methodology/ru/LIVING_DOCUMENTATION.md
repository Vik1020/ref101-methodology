# Руководство по живой документации

**Версия:** 1.1.0
**Последнее обновление:** 2026-01-04
**Назначение:** Методология накопления BC/AC документации с единым источником истины

> **Это подпроцесс фазы APPLY_DELTAS.** См.: [WORKFLOW.md](../WORKFLOW.md)
...
- Контекстное окно LLM заполняется избыточной историей

---

## Решение: Инкрементальная машина состояний
...
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  Уровень 1: OVERVIEW.md (~200 строк)                        │
│  ├── Описание продукта                                       │
│  ├── Список доменов                                          │
│  └── Ключевые возможности                                    │
│                                                              │
│  Уровень 2: BC/AC_DOMAIN_*.md (~500-1000 строк каждый)      │
...
│                                                              │
│  Уровень 3: deltas/ (архив изменений)                       │
│  └── v{X.Y.Z}/                                              │
...
2. **DOMAIN_*.md = master-документы**: текущее полное состояние по домену
3. **OVERVIEW.md = навигатор**: высокоуровневый обзор для быстрого старта
4. **deltas/ = архив**: для восстановления при сбое и журнала аудита
...
| Полная перегенерация | O(n) | Только при восстановлении |
...
```
docs/
├── TOOLS_INDEX.md                 # Реестр нескольких инструментов
├── OVERVIEW_{tool}.md             # Обзор по инструменту
│
├── domains/                       # Уровень 2
...
│
└── archive/{tool}/                # Архив по инструменту
    ├── business/
    └── analytics/
```

### Проекты с несколькими инструментами

Для репозиториев с несколькими инструментами добавьте префикс `{tool}`:
...
**Правило:** Один `docs/` на проект, НЕ на инструмент.

> См.: [WORKFLOW.md → Multi-Tool Projects](../WORKFLOW.md#multi-tool-projects)
...
```
┌─────────────┐     ┌─────────────┐     ┌─────────────┐
│  1. Создать │────▶│  2. Применить│────▶│  3. Проверить│
│    Delta    │     │   к Domain  │     │  и архивировать│
└─────────────┘     └─────────────┘     └─────────────┘
```
...
**Шаг 3: Проверить и архивировать**
```bash
...
git commit -m "docs: Применены delta v1.9.0 к доменам"
```

### При чтении документации

```
Стратегия контекста LLM:

1. Всегда загружать: OVERVIEW.md (~200 строк)
2. По запросу: BC/AC_DOMAIN_*.md нужного домена (~1000 строк)
3. Никогда: deltas/ (только для восстановления)

Итого контекст: ~1200 строк вместо ~5000+ строк старых BC/AC
```
...
BC_DOMAIN_ui:
  ✓ v1.0.0 применён
  ✓ v1.5.0 применён
  ✗ v1.7.0 ОТСУТСТВУЕТ — delta существует в deltas/v1.7.0/BC_delta_ui.md
  ✓ v1.8.0 применён (обнаружен пропуск!)
...
|----------|-------|----------|
| Строки в DOMAIN файле | > 1000 | Разделить на поддомены |
| Количество фич | > 30 | Разделить по функциональности |
...
4. Обновляет изменённые features
5. Помечает устаревшие features как deprecated
...
**Когда использовать:**
- При обнаружении пропуска в applied_deltas
...
Заменить ссылки на старые BC/AC на новые domain файлы.
...
|----------|----------------------|----------------------|
| Разрастание доменов | Реестр доменов | Агрегаторы доменов |
| Рост размеров файлов | Многофайловая структура | Разделение доменов |
| Дублирование контента | Ссылочный подход | Только delta-документы |
| Сложность навигации | Иерархическая структура | 3-уровневая архитектура |
| Битые ссылки | Инструменты валидации | Цепочка based_on |
| Устаревшая документация | Метрики здоровья | Отслеживание applied_deltas |

**Когда использовать что:**
- DOCUMENTATION_SCALING: общие принципы масштабирования
- LIVING_DOCUMENTATION: конкретный механизм для накопления BC/AC
...
- [ ] Создать структуру docs/domains/
- [ ] Создать структуру docs/releases/
- [ ] Создать OVERVIEW.md
...
- [ ] Обновить CLAUDE.md с новым воркфлоу
- [ ] Обновить шаблон RELEASE
